import { a as EmbedBlot$1 } from "./quill-Bj-B_DhC.js";
//#region node_modules/@mgreminger/quill-image-resize-module/dist/index.es.js
...
function require_baseSetToString() {
	...
	_baseSetToString = !defineProperty ? identity : function(func, string) {
...
function require_cloneBuffer() {
	...
	(function(module, exports$1) {
		...
		var Buffer2 = freeModule && freeModule.exports === freeExports ? root.Buffer : void 0, allocUnsafe = Buffer2 ? Buffer2.allocUnsafe : void 0;
...
function requireIsArguments() {
	...
	isArguments_1 = baseIsArguments(/* @__PURE__ */ (function() {
		return arguments;
...
function requireIsTypedArray() {
	...
	isTypedArray_1 = nodeIsTypedArray ? baseUnary(nodeIsTypedArray) : baseIsTypedArray;
...
function require_customDefaultsMerge() {
	...
	var baseMerge = require_baseMerge(), isObject = requireIsObject();
...
function requireMergeWith() {
	...
	mergeWith_1 = require_createAssigner()(function(object, source, srcIndex, customizer) {
...
function requireDefaultsDeep() {
	...
	var apply = require_apply(), baseRest = require_baseRest(), customDefaultsMerge = require_customDefaultsMerge(), mergeWith = requireMergeWith();
	defaultsDeep_1 = baseRest(function(args) {
		args.push(void 0, customDefaultsMerge);
		return apply(mergeWith, void 0, args);
	});
	return defaultsDeep_1;
}
var defaultsDeep = /* @__PURE__ */ getDefaultExportFromCjs(requireDefaultsDeep());
var DefaultOptions = {
	modules: [
		"DisplaySize",
		"Resize",
...
var DisplaySize = class extends BaseModule {
	constructor() {
		super(...arguments);
		__publicField(this, "display", null);
		__publicField(this, "onCreate", () => {
			var _a;
			this.display = document.createElement("div");
			Object.assign(this.display.style, this.options.displayStyles);
			(_a = this.overlay) == null || _a.appendChild(this.display);
		});
		__publicField(this, "onDestroy", () => {});
		__publicField(this, "onUpdate", () => {
			...
			if (size[0] > 120 && size[1] > 30) Object.assign(this.display.style, {
				right: "4px",
				bottom: "4px",
				left: "auto"
			});
		...
		__publicField(this, "getCurrentSize", () => {
			if (this.img) return [this.img.width, Math.round(this.img.width / this.img.naturalWidth * this.img.naturalHeight)];
			else return [0, 0];
		});
	}
};
var Resize = class extends BaseModule {
	constructor() {
		super(...arguments);
		__publicField(this, "boxes", []);
		__publicField(this, "dragBox", null);
		...
		__publicField(this, "onDestroy", () => {
			for (const box of this.boxes) box.removeEventListener("mousedown", this.handleMousedown, false);
			this.setCursor("");
		});
		__publicField(this, "positionBoxes", () => {
		...
		__publicField(this, "addBox", (cursor) => {
			var _a;
			...
			(_a = this.overlay) == null || _a.appendChild(box);
			this.boxes.push(box);
		});
		__publicField(this, "handleMousedown", (evt) => {
			if (evt.target instanceof HTMLElement) {
				this.dragBox = evt.target;
				if (evt.type === "touchstart") this.dragStartX = evt.changedTouches[0].clientX;
				else this.dragStartX = evt.clientX;
				if (this.img) this.preDragWidth = this.img.width || this.img.naturalWidth;
			...
			} else console.warn("mousedown target is not an HTMLElement");
		});
		__publicField(this, "handleMouseup", (evt) => {
			evt.stopPropagation();
			this.setCursor("");
		...
		__publicField(this, "handleDrag", (evt) => {
			if (!this.img) return;
			let clientX;
			if (evt.type === "touchmove") clientX = evt.changedTouches[0].clientX;
			else clientX = evt.clientX;
			const deltaX = clientX - this.dragStartX;
			if (this.dragBox === this.boxes[0] || this.dragBox === this.boxes[3]) this.img.width = Math.max(Math.round(this.preDragWidth - deltaX), this.options.minWidth);
			else this.img.width = Math.max(Math.round(this.preDragWidth + deltaX), this.options.minWidth);
			this.requestUpdate();
		});
		__publicField(this, "setCursor", (value) => {
...
var AltText = class extends BaseModule {
	constructor() {
		super(...arguments);
		...
		__publicField(this, "handleChange", () => {
			if (this.img && this.textarea) this.img.setAttribute("alt", this.textarea.value);
		});
		__publicField(this, "stopEvent", (evt) => {
			evt.stopPropagation();
		});
...
var ImageResize = class {
	constructor(quill, options = {}) {
		...
		__publicField(this, "options");
		__publicField(this, "moduleClasses");
		__publicField(this, "modules");
		__publicField(this, "img", null);
		__publicField(this, "overlay", null);
		__publicField(this, "initializeModules", () => {
			this.removeModules();
			this.modules = this.moduleClasses.map((ModuleClass) => {
				if (typeof ModuleClass === "string") return new knownModules[ModuleClass](this);
				else return new ModuleClass(this);
			});
		...
		__publicField(this, "removeModules", () => {
			this.modules.forEach((module) => {
				module.onDestroy();
			});
			this.modules = [];
		...
		__publicField(this, "handleClick", (event) => {
			if (event.target instanceof HTMLImageElement) {
				const blot = this.quill.constructor.find(event.target);
				if (blot instanceof EmbedBlot$1) this.quill.setSelection(blot.offset(this.quill.scroll), blot.length());
			}
		});
		__publicField(this, "handleSelectionChange", (range) => {
			let firstImage = null;
			if (range) {
				const blots = this.quill.scroll.descendants(EmbedBlot$1, range.index, range.length);
				for (const blot of blots) if (blot.domNode instanceof HTMLImageElement) firstImage = blot.domNode;
			}
			if (firstImage) this.show(firstImage);
			else if (this.img) {
				if (this.overlay && this.overlay.contains(document.activeElement)) return;
				this.hide();
			}
		});
		__publicField(this, "handleTextChange", () => {
			if (this.img) {
				if (!this.quill.constructor.find(this.img)) this.hide();
			}
		});
		__publicField(this, "show", (img) => {
			this.img = img;
			this.showOverlay();
		...
		__publicField(this, "showOverlay", () => {
			...
			(_a = this.quill.root.parentNode) == null || _a.appendChild(this.overlay);
			this.repositionElements();
		});
		__publicField(this, "hideOverlay", () => {
			var _a;
			if (!this.overlay) return;
			(_a = this.quill.root.parentNode) == null || _a.removeChild(this.overlay);
			this.overlay = null;
			this.setUserSelect("");
			this.quill.root.removeEventListener("keydown", this.handleKeyboardShortcuts);
		});
		__publicField(this, "repositionElements", () => {
			if (!this.overlay || !this.img) return;
			...
			} else console.warn("parentNode is not an HTMLElement");
		});
		__publicField(this, "hide", () => {
			this.hideOverlay();
			this.removeModules();
		...
		__publicField(this, "setUserSelect", (value) => {
			this.quill.root.style.setProperty("user-select", value);
			document.documentElement.style.setProperty("user-select", value);
		});
		__publicField(this, "handleKeyboardShortcuts", (event) => {
			var _a;
			if (event.defaultPrevented) return;
			switch (event.key) {
				case "+":
					if (this.img) {
						this.img.width = Math.max(this.img.width + this.options.keyboardSizeDelta, this.options.minWidth);
						this.onUpdate();
					} else return;
					break;
				case "-":
				...
				case "A":
				case "F2":
					const altInput = (_a = this.overlay) == null ? void 0 : _a.querySelector("textarea");
					if (altInput) altInput.focus({ preventScroll: false });
					else return;
					break;
				case "Escape":
					this.hide();
		...
		if (options.modules) moduleClasses = options.modules.slice();
		this.options = defaultsDeep({}, options, DefaultOptions);
		if (moduleClasses !== false) this.options.modules = moduleClasses;
