...
def create_question_in_form(
    ...
    for opt in payload.options:
        db.add(models.QuestionOption(question_id=question.id, label=opt.label, value=opt.value, order_index=opt.order_index, is_correct=opt.is_correct, is_other=opt.is_other))
...
def create_option(
    ...
    option = models.QuestionOption(question_id=question.id, label=payload.label, value=payload.value,
                                   order_index=payload.order_index, is_correct=payload.is_correct, is_other=payload.is_other)
    db.add(option)
    db.commit()
    db.refresh(option)
    return option
